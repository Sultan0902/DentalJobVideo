...
import com.vishant.DentalJobVideo.adapter.QB.calling.OpponentsFromCallAdapter;
...
//          for correct removing item in adapter
    ...
    private void updateSwitchCameraIcon(final MenuItem item) {
        if (isCurrentCameraFront) {
            ...
            item.setIcon(R.mipmap.ic_camera_front);
        } else {
            Log.d(TAG, "CameraRear now!");
            item.setIcon(R.mipmap.ic_camera_rear);
        }
    }
